...
export const Graph2D = (props) => {
    ...
    const handleMouseMove = (e) => {
        ...
        if (mouseDownFunction)
        {
            ...
            const x = toCartesianX(pos.x);
            const y = mouseDownFunction[0](x);
            const pixel_y = toPixelsY(y);
        ...
        if (!mouseDown){
            ...
            try{
                const pos = getMousePos(e);
                const x = toCartesianX(pos.x);
                const func = props.functions[0][0]; // Assumes there's at least one function
                const y = func(x);
                const pixelY = toPixelsY(y);
                setHoverPoint(new Vector(pos.x, pixelY));
            }
            catch(err){
                console.warn(`❌ error on hover, error: `, {err});
            }
            return;
        }
        else{
